...
new class extends Component
{
    ...
    public $search;
    public $parent;
    public OptionValue $model;
    public Option $option;

    #[\Livewire\Attributes\Layout('layouts.dashboard')]
    public function mount(Option $option,OptionValue $model)
    {
        abort_if(!auth()->user()->can('categories.view'), 403);
        $this->model=$model;
        $this->option=$option;
        $this->info['header']='لیست مقدار ویژگی ها';
        $this->info['create']='افزودن مقدار ویژگی';
        $this->info['delete']='حذف مقدار ویژگی';
        $this->info['personal']='مقدار ویژگی';
    ...
    {
        $query = $this->model->where('option_id',$this->option->id)->where(function ($query) {
    ...
    {
        if ($action == 'create'){
            $this->resetExcept('model','info','data','option');
        }else{
            $this->resetExcept(['selectItem','model','info','data','option']);
    ...
    public function messages()
    {
        return [
            ...
            // title
            'title.required' => 'وارد کردن نام مقدار ویژگی الزامی است.',
            'title.string'   => 'نام مقدار ویژگی باید متن باشد.',
            'title.min'      => 'نام مقدار ویژگی باید حداقل ۲ کاراکتر باشد.',
            'title.max'      => 'نام مقدار ویژگی نباید بیشتر از ۲۰۰ کاراکتر باشد.',
            ...
            //sort
            'sort.required' => 'وارد کردن جایگاه مقدار ویژگی الزامی است.',
                    ...
                    <form wire:submit.prevent="save()"  id="save">
                        <div class="row">
                            ...
                            <div class="col-xl-4">
                                ...
                                <input wire:model.lazy="sort" type="number" class="form-control @error('sort') is-invalid @enderror" id="input-rounded" placeholder="لطفا جایگاه {{$info['personal'] ?? ''}} را وارد کنید">
                                @error('sort')
                                <div  class="invalid-feedback">
                                    {{$message}}
                                </div>
