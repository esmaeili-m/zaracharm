...
new class extends Component
{
    ...
    public function change_status($id)
    {
        ...
        $this->dispatch(
            ...
            type: 'success',
            title: 'عملیات موفق',
            text: 'دسته بندی موفقیت آپدیت شد.'
        );
    }
    ...
    {
        if ($action == 'create'){
            $this->resetExcept('model','info','data','tags');
        }else{
            $this->resetExcept(['selectItem','model','info','data','tags']);
    ...
    public function messages()
    {
        return [
            ...
            'slug' => [
                ...
                'string' => 'اسلاگ باید رشته باشد.',
                'max' => 'اسلاگ نباید بیشتر از ۲۰۰ کاراکتر باشد.',
                'regex' => 'اسلاگ فقط می‌تواند شامل حروف، اعداد، خط تیره و زیرخط باشد.',
                        ...
                        <form wire:submit.prevent="save()"  id="save">
                            ...
                            <div class="col-xl-4">
                                ...
                                <select wire:model.lazy="parent_id" class="form-control @error('parent_id') is-invalid @enderror" data-trigger="" id="choices-single-default" title="choices-single-default">
                                    ...
                                    @foreach(\App\Models\Category::whereNull('parent_id')->where('status',1)->pluck('title','id') as $key => $role)
                                        <option value="{{$key}}">
                                            {{$role}}
                                        </option>
                                    @endforeach
