...
new class extends Component
{
    ...
    {
        $this->colors = ['blue', 'red', 'green', 'purple', 'pink', 'cyan', 'yellow', 'indigo'];
        $this->data = $data;
        if ($data['mode'] == 'sales') {
            ...
            $this->categories = Category::query()
                ...
                ->selectRaw('SUM(products.views) as total_views')
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        <span class="px-3 py-1 bg-primary/10 text-primary text-[10px] font-black rounded-full tracking-widest uppercase">دسترسی سریع</span>
                    ...
                    <h2 class="text-4xl font-black text-gray-900 dark:text-white tracking-tight">
                        ...
                        <span class="absolute bottom-2 inset-x-0 h-3 bg-primary/20 -z-10 rounded-full"></span>
                    </span>
                    </h2>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-6">
                    @foreach($categories ?? [] as $category)
                        <a href="#" class="group relative flex flex-col items-center">
                            <div class="relative w-full aspect-square max-w-[160px] flex items-center justify-center mb-6">
                                <div class="absolute inset-0 bg-white/60 dark:bg-white/[0.03] backdrop-blur-md rounded-[3rem] border border-white dark:border-white/10 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] transition-all duration-700 group-hover:-translate-y-4 group-hover:bg-white/80 dark:group-hover:bg-white/[0.07] group-hover:shadow-lg group-hover:shadow-primary/20 group-hover:border-primary/30"></div>

                                <div class="relative z-10 p-6 flex flex-col items-center transition-transform duration-700 group-hover:scale-110">
                                    <div class="absolute inset-0 bg-primary/20 blur-[30px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                    <img src="{{$category->featuredImageUrl}}" alt="{{$category->title}}" class="relative z-10 w-25 h-25 object-contain drop-shadow-2xl">
                                </div>
                            </div>
                            <span class="text-[13px] font-black text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors duration-300">{{$category->title}} </span>
                            <div class="mt-4 w-1.5 h-1.5 bg-primary rounded-full opacity-0 group-hover:opacity-100 group-hover:scale-[3] transition-all duration-500"></div>
                        </a>
                    @endforeach
                </div>
            @elseif((int) $data['view'] == 2)
                <section class="container w-full mb-32 px-6" dir="rtl">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        @foreach($categories ?? [] as $category)
                            <div class="group relative min-h-[400px] overflow-hidden rounded-[4rem] border border-white/60 dark:border-none shadow-lg shadow-blue-500/5">
                                ...
                                <div class="relative z-10 h-full p-12 flex flex-col justify-between">
                                    <div class="max-w-xs space-y-6">
                                        @foreach($category->tags ?? []  as $tag)
                                            <span class="inline-block px-4 py-2 bg-blue-600/10 dark:bg-blue-400/20 text-blue-600 dark:text-blue-400 rounded-2xl text-[10px] font-black uppercase tracking-widest">{{$tag->title}}</span>
                                        ...
                                        <a href="#" class="inline-flex items-center gap-3 text-gray-900 dark:text-white font-black text-sm group-hover:gap-5 transition-all">
                                            مشاهده محصولات
                                            <svg class="w-5 h-5 bg-blue-600 text-white rounded-full p-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg>
                                        </a>
                                    </div>
