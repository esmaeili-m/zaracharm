...
new class extends Component
{
    ...
    {
        $productIds = Product::query()->with('categories')
    ...
    protected function loadProducts()
    {
        ...
        $this->products = match ($this->data['mode']) {
            ...
            'latest' => Product::query()->with('categories')
                ->latest()
                ->limit($limit)
                ->get(),
            ...
            'views' => Product::query()->with('categories')
                ->orderByDesc('views')
                ->limit($limit)
                ->get(),

            'random' => Product::query()->with('categories')
                ->inRandomOrder()
                ->limit($limit)
                ->get(),

            'manual' => Product::query()->with('categories')
                ->whereIn(
                    'id',
                ...
                <div id="product-grid" class="grid pb-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    @foreach($products as $product)
                        @php
                            ...
                            if ($product?->displayVariant?->compare_price > $product?->displayVariant?->price) {
                                $discount = round(
                                    (($product?->displayVariant?->compare_price - $product?->displayVariant?->price) / $product?->displayVariant?->compare_price) * 100
                                );
                            }
                        @endphp
                        <div class="product-card group relative bg-white/70 dark:bg-white/[0.03] backdrop-blur-md rounded-[2.5rem] border border-gray-200 dark:border-white/10 p-2 transition-all duration-500 hover:shadow-lg hover:shadow-brown-600/20 hover:-translate-y-2"  data-categories="{{ $product->categories->pluck('slug')->implode(' ') }}">
                            <div class="flex h-[220px]">
                                <div class="w-2/5 relative bg-gradient-to-br from-gray-100 to-transparent dark:from-white/5 dark:to-transparent rounded-[2rem] overflow-hidden flex items-center justify-center m-1 transition-all duration-500 group-hover:scale-[0.98]">
                                    <img src="{{$product->featuredImageUrl}}" class="w-32 h-32 object-contain drop-shadow-md transition-transform duration-700 group-hover:scale-110 group-hover:-rotate-6" alt="iPhone 15">
                                    @if($discount > 0)
                                ...
                                <div class="w-3/5 p-5 flex flex-col justify-between">
                                    <div>
                                        <div class="flex justify-between items-start">
                                            <span class="text-[10px] font-bold text-brown-600 dark:text-brown-400 tracking-tighter opacity-80 mb-1 block">{{$product->title}}</span>
                                    ...
                                    <div class="mt-auto">
                                        <div class="mb-3 text-left">
                                            <p class="text-[10px] text-gray-400 line-through mb-0.5">{{number_format($product?->displayVariant?->compare_price)}}</p>
                                        ...
                                        <button class="w-full py-3 bg-brown-500 text-white rounded-xl text-[11px] font-black shadow-lg shadow-brown-500/20 hover:bg-brown-700 transition-all flex items-center justify-center gap-2 group/btn">
                                            <span>خرید سریع</span>
                                            <svg class="w-4 h-4 transition-transform group-hover:translate-x-[-3px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="3" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="container mx-auto relative z-10">
                ...
                <div id="product-grid" class="grid pb-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    @foreach($products as $product)
                        ...
                        @endphp
                        <div class=" group relative bg-white/70 dark:bg-white/[0.03] backdrop-blur-md rounded-[2.5rem] border border-gray-200 dark:border-white/10 p-2 transition-all duration-500 hover:shadow-lg hover:shadow-brown-600/20 hover:-translate-y-2"  data-categories="{{ $product->categories->pluck('slug')->implode(' ') }}">
                            <div class="flex h-[220px]">
            ...
            <div class="container pb-7 relative z-10">
                ...
                <div class="flex items-end justify-between mb-6 gap-4 flex-wrap">
                    <div class="flex items-center gap-6">
                        ...
                        <div>
                            <h2 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">پرفروش‌ترین‌ها</h2>
                    ...
                    <div class="swiper productSwiper !overflow-visible">
                        ...
                        <div class="swiper-slide h-auto p-4">
                            <div class="group relative h-full pt-12">
                                <div class="absolute inset-0 bg-white/80 dark:bg-[#0a0a0a]/40 backdrop-blur-[20px] rounded-[3rem] border border-gray-100 dark:border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.02)] transition-all duration-700 group-hover:border-blue-500/50 dark:group-hover:shadow-[0_0_60px_rgba(37,99,235,0.12)]"></div>
                                ...
                                <div class="relative p-7 flex flex-col h-full z-10 transition-transform duration-500 group-hover:-translate-y-4">
                                    ...
                                    <div class="relative mb-8 flex items-center justify-center min-h-[180px]">
                                        <div class="absolute w-40 h-40 bg-blue-500/20 dark:bg-indigo-500/20 blur-[70px] rounded-full opacity-0 group-hover:opacity-100 transition-all duration-1000"></div>

                                        <img  src="{{$product->featuredImageUrl}}"
                                              class="relative z-10 w-full h-44 object-contain transition-all duration-700 group-hover:scale-110 group-hover:drop-shadow-[0_15px_35px_rgba(37,99,235,0.3)]"
                                              alt="{{$product->title}}">
                                    ...
                                    <h3 class="text-[15px] font-black text-gray-800 dark:text-zinc-100 mb-6 line-clamp-2 leading-7 h-14 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                        {{$product->title}}
                                    </h3>

                                    <div class="flex items-center justify-between mt-auto pt-5 border-t border-gray-100 dark:border-white/5">
                                        <div class="flex flex-col gap-1">
                                            <span class="text-[11px] text-gray-400 dark:text-zinc-500 line-through tabular-nums leading-none">{{number_format($product?->displayVariant?->compare_price)}}</span>
                                            <div class="flex items-center gap-1.5">
                                                <span class="text-2xl font-black text-gray-900 dark:text-white tracking-tighter tabular-nums">{{number_format($product?->displayVariant?->price)}}</span>
                ...
                <div class="flex justify-center gap-4 mt-10">
                    <div class="prod-prev w-14 h-14 rounded-2xl bg-white/50 dark:bg-white/5 dark:text-white border border-white dark:border-white/10 flex items-center justify-center cursor-pointer hover:bg-blue-600 hover:text-white transition-all shadow-lg group">
                    ...
                    <div class="prod-next w-14 h-14 rounded-2xl bg-white/50 dark:bg-white/5 dark:text-white border border-white dark:border-white/10 flex items-center justify-center cursor-pointer hover:bg-blue-600 hover:text-white transition-all shadow-lg group">
