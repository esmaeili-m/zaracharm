...
<div class="relative bg-white/40 dark:bg-white/[0.03] backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[2.5rem] overflow-hidden shadow-lg shadow-gray-200/50 dark:shadow-none transition-all duration-500">
    <div class="js-collapse-header flex items-center justify-between p-7 cursor-pointer select-none group/header">
        ...
        <div class="flex items-center gap-3">
            <span class="text-[10px] font-black text-brown-500 bg-brown-500/10 px-3 py-1 rounded-full uppercase tracking-widest">{{ $category->title }}</span>
            ...
            <ul class="px-7 pb-8 space-y-1">
                @foreach($childCategories as $child)
                    <li class="group/category">
                        <label class="flex items-center justify-between p-3 rounded-2xl hover:bg-brown-500/5 dark:hover:bg-brown-500/10 cursor-pointer transition-all border border-transparent hover:border-brown-500/20">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 group-hover/category:bg-brown-500 group-hover/category:text-white transition-all">
                                ...
                                <span class="text-xs font-bold text-gray-700 dark:text-gray-300 group-hover/category:text-brown-600 transition-colors">
                                    {{ $child->title }}
                                </span>
                            </div>
                            ...
                            <div class="relative flex items-center">
                                <input
                                    ...
                                    class="peer appearance-none w-5 h-5 rounded-lg border-2 border-gray-300/70 dark:border-white/20 checked:bg-brown-500 checked:border-brown-500 transition-all cursor-pointer"
                                >
...
@if($priceCeil > 0)
    <div class="js-price-range-container relative bg-white/40 dark:bg-white/[0.03] backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[2.5rem] overflow-hidden shadow-lg shadow-gray-200/50 dark:shadow-none transition-all duration-500">
        ...
        <div class="js-collapse-content overflow-hidden transition-all duration-500 ease-in-out" style="max-height: 1000px; opacity: 1;">
            <div class="px-7 pb-8 pt-3">
                ...
                {{--
                    اسلایدر دو‌دستگیره‌ی خودکفا با Alpine.js.

                    چرا مستقل نوشته شده و از دو <input type="range"> روی‌هم قبلی استفاده نمی‌کند:
                    دو رنج‌اینپوت روی‌هم گذاشته‌شده، مخصوصاً داخل یک صفحه‌ی RTL، به‌خاطر تداخل pointer-events
                    و جهت محاسبه‌ی مقدار، رایج‌ترین دلیل خراب‌شدن این الگوی UI است. این پیاده‌سازی به‌جایش
                    دو "دستگیره‌ی" مجزا روی یک ترک با ریاضیِ ساده‌ی LTR (با dir="ltr" مستقل از جهت صفحه) دارد.

                    هماهنگی با Livewire 4:
                    - wire:ignore روی خودِ ترک، تا re-render لایوایر بعد از هر اکشن (مثل تغییر یک فیلتر دیگر)
                      این ترک را از نو نسازد و state درگ فعلی را خراب نکند یا لیسنر تکراری ایجاد نکند.
                    - state محلی (min/max داخل Alpine) در حین درگ فقط بصری آپدیت می‌شود؛ هیچ درخواستی به
                      سرور نمی‌رود.
                    - فقط در لحظه‌ی رها کردن دستگیره (pointerup/touchend)، یک‌بار updatePriceRange(min, max)
                      روی کامپوننت Livewire صدا زده می‌شود؛ یعنی دقیقاً یک Request برای کل عملیات درگ.
                    - $watch روی $wire.minPrice/$wire.maxPrice باعث می‌شود اگر مقدار از راه دیگری (مثلاً
                      تایپ در اینپوت عددیِ «از»/«تا») عوض شد، دستگیره‌های بصری هم خودشان را sync کنند.
                --}}
                <div
                    ...
                    max: {{ $maxPrice ?? $priceCeil }},
                })"
                    wire:ignore
                    dir="ltr"
                    class="relative w-full mx-auto h-1.5 bg-gray-200 dark:bg-white/10 rounded-full mb-10 mt-4 select-none"
                >
                    <div
                        class="absolute h-full bg-brown-500 rounded-full pointer-events-none"
                        :style="`right: ${100 - maxPercent}%; left: ${minPercent}%;`"
                    ></div>

                    <div
                        class="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-6 h-6 bg-white dark:bg-zinc-900 border-2 border-brown-500 rounded-full shadow-md cursor-grab active:cursor-grabbing z-20 touch-none"
                ...
                <div class="space-y-4">
                    <div class="group/in relative flex items-center bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[1.25rem] px-4 py-3 transition-all focus-within:border-brown-500/50 focus-within:bg-white dark:focus-within:bg-white/10">
                        <span class="text-[11px] font-bold text-gray-400 ml-3">از</span>
                        <input
                            type="number"
                            wire:model.live.debounce.600ms="minPrice"
                            class="js-min-price-input w-full bg-transparent border-none focus:ring-0 text-sm font-black text-gray-700 dark:text-white text-left p-0"
                        >
                    </div>
                    <div class="group/in relative flex items-center bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[1.25rem] px-4 py-3 transition-all focus-within:border-brown-500/50 focus-within:bg-white dark:focus-within:bg-white/10">
                        ...
                        <input
                            ...
                            class="js-max-price-input w-full bg-transparent border-none focus:ring-0 text-sm font-black text-gray-700 dark:text-white text-left p-0"
                        >
                    </div>
                </div>

                <button wire:click="applyPriceRange" class="w-full mt-8 py-4 bg-brown-500 hover:bg-brown-600 text-white rounded-[1.5rem] text-xs font-black transition-all active:scale-95">تایید محدوده قیمت</button>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            // جلوگیری از رجیستر تکراری اگر این partial (offcanvas + aside) دو بار روی صفحه include شود.
            if (window.__dualRangeSliderRegistered) {
                return;
            }
            window.__dualRangeSliderRegistered = true;

            Alpine.data('dualRangeSlider', (config) => ({
                floor: config.floor,
                ceil: config.ceil,
                ...
                init() {
                    this.min = this.clamp(this.min);
                    this.max = this.clamp(this.max);

                    // اگر minPrice/maxPrice از جای دیگری (مثلاً اینپوت عددی) عوض شد، دستگیره‌ها sync شوند.
                    this.$watch('$wire.minPrice', (value) => {
                        if (value !== null && this.dragging !== 'min') {
                            this.min = this.clamp(value);
                        }
                    });

                    this.$watch('$wire.maxPrice', (value) => {
                        if (value !== null && this.dragging !== 'max') {
                            this.max = this.clamp(value);
                        }
                    });
                },

                clamp(value) {
                    return Math.min(this.ceil, Math.max(this.floor, value));
                },

                get range() {
                    return Math.max(1, this.ceil - this.floor);
                },

                get minPercent() {
                    return ((this.min - this.floor) / this.range) * 100;
                },

                get maxPercent() {
                    return ((this.max - this.floor) / this.range) * 100;
                },

                valueFromClientX(clientX) {
                    const track = this.$root.getBoundingClientRect();
                    let percent = (clientX - track.left) / track.width;
                    percent = Math.min(1, Math.max(0, percent));

                    let value = this.floor + percent * this.range;
                ...
                startDrag(handle, event) {
                    ...
                    const move = (e) => {
                        ...
                        const value = this.valueFromClientX(clientX);
                    ...
                    const stop = () => {
                        ...
                        // فقط همین یک درخواست، در لحظه‌ی رها کردن دستگیره.
                        this.$wire.updatePriceRange(this.min, this.max);
                    };
...
@if($brands->isNotEmpty())
    <div class="relative bg-white/40 dark:bg-white/[0.03] backdrop-blur-md border border-white/40 dark:border-white/10 rounded-[2.5rem] overflow-hidden shadow-lg shadow-gray-200/50 dark:shadow-none transition-all duration-500">
        <div class="js-collapse-header flex items-center justify-between p-7 cursor-pointer select-none group/header">
            ...
            <div class="flex items-center gap-3">
            <span class="text-[10px] font-black text-brown-500 bg-brown-500/10 px-3 py-1 rounded-full uppercase tracking-widest">
        ...
        <div class="js-collapse-content overflow-hidden transition-all duration-500 ease-in-out" style="max-height: 1000px; opacity: 1;">
            <div class="px-7 pb-8 space-y-5">
                <div class="relative group/search">
                    <input type="text" class="js-brand-search w-full bg-white/60 dark:bg-white/10 border border-gray-200 dark:border-white/20 rounded-2xl py-3.5 pr-11 pl-4 text-xs font-bold text-gray-800 dark:text-white outline-none transition-all focus:border-brown-500/50 focus:ring-4 focus:ring-brown-500/5" placeholder="جستجوی برند...">
                ...
                <ul class="js-brands-list space-y-1 max-h-72 overflow-y-auto custom-scrollbar pl-2">
                    @foreach($brands as $brand)
                        <li class="group/brand">
                            <label class="flex items-center justify-between p-3 rounded-2xl hover:bg-brown-500/5 dark:hover:bg-brown-500/10 cursor-pointer transition-all border border-transparent hover:border-brown-500/20">
                                <span class="text-xs font-bold text-gray-700 dark:text-gray-300 group-hover/brand:text-brown-600 transition-colors">{{ $brand->title }}</span>
                                <div class="relative flex items-center">
                                    <input
                                        type="checkbox"
                                        value="{{ $brand->id }}"
