...
function updateCartUI() {
    ...
    if (rows.length === 0) {
        if (emptyMsg) emptyMsg.classList.replace('hidden', 'flex');
        ...
        if (emptyMsg) emptyMsg.classList.replace('flex', 'hidden');
...
function initMobileSearchModal() {
    ...
    const openSearch = () => {
        ...
        document.body.style.overflow = 'hidden';
        setTimeout(() => modalInput.focus(), 400);
    };
...
class CategoryFilters {
    ...
    initUniversalCollapse() {
        document.addEventListener('click', (e) => {
            ...
            const isOpen = content.style.maxHeight && content.style.maxHeight !== '0px' && content.style.maxHeight !== '';
    ...
    initAllPriceRanges() {
        ...
        containers.forEach(container => this.initPriceRange(container));
    }
...
/* =========================================================
   18. CHECKOUT ADDRESS MANAGEMENT
   ========================================================= */
...
function initShippingCost() {
    ...
    const baseProductPrice = 125000000;
    const totalDiscount = 2000000;
...
/* =========================================================
   27. TERMS & CONDITIONS NAVIGATION (SCROLLSPY) - OPTIMIZED
   ========================================================= */

/**
 * TERMS & CONDITIONS NAVIGATION (SCROLLSPY)
 * Optimized using IntersectionObserver instead of scroll event for better performance
 */
function initTermsNavigation() {
    ...
    }, { rootMargin: '-100px 0px -200px 0px', threshold: 0.2 });
...
/* =========================================================
   28. PRODUCT QUANTITY CONTROLLER
   ========================================================= */


/**
 * PRODUCT QUANTITY CONTROLLER
 * Manages quantity changes in quick view modal and product pages
 */

/**
 * Changes product quantity by delta value
...
function changeQty(delta) {
    ...
    // Enforce min/max limits (1 to 99)
    if (newValue < 1) newValue = 1;
    if (newValue > 99) newValue = 99;

    qtyInput.value = newValue;

    // Trigger a custom event for other parts of the app that might need to know quantity changed
    const quantityChangeEvent = new CustomEvent('quantityChanged', {
        detail: { quantity: newValue }
    });
    qtyInput.dispatchEvent(quantityChangeEvent);

    // Optional: Update any associated price display
    if (typeof updatePriceByQuantity === 'function') {
        updatePriceByQuantity(newValue);
    }
}

/**
 * Updates price display based on quantity (for product modals)
...
function updatePriceByQuantity(quantity) {
    ...
    const basePriceElement = document.getElementById('modal-base-price');

    if (!priceElement || !basePriceElement) return;

    const basePrice = parseInt(basePriceElement.dataset.price);
    if (isNaN(basePrice)) return;

    const totalPrice = basePrice * quantity;
    priceElement.innerHTML = `${totalPrice.toLocaleString('fa-IR')} <span class="text-[10px]">تومان</span>`;
}
